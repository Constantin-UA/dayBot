...
async def get_ai_forecast(symbol: str, price: float, current_vwap: float, vwap_distance_pct: float, 
    ...
    if total_signals >= 3 and win_rate < 40.0:
        reflection_block = f"КРИТИЧНА УВАГА: Твій Win Rate за 24 години впав до {win_rate:.1f}%. Ринок у стадії жорсткого 'запилу' (зняття ліквідності). ТИ ЗОБОВ'ЯЗАНИЙ ЗМЕНШИТИ АГРЕСІЮ. Заходь в угоду ТІЛЬКИ якщо математичне співвідношення Risk/Reward становить мінімум 1:1.5 або краще. Якщо гірше — видавай вердикт ПОЗА РИНКОМ."
